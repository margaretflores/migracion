...
namespace appFew.role
{
    public partial class roles : System.Web.UI.Page
    {
        ...
        private List<GRROAP> MaestroRoles
        {
            get
            {
                List<GRROAP> dt = HttpContext.Current.Session[Constantes.NOMBRE_SESION_MAE_ROLES] as List<GRROAP>;
                if (dt == null)
                {
            ...
            set
            {
                HttpContext.Current.Session[Constantes.NOMBRE_SESION_MAE_ROLES] = value;
            }
        }
        ...
        private void CargaRoles(int pageindex)
        {
            ...
            try
            {
                ...
                argumentos.CODOPE = CodigoOperacion.OBTIENE_MAESTRO_ROLES;
        ...
        private void MuestraDatos()
        {
            ...
            if (row != null)
            {
                ...
                //LENAR TABPAGES
                List<GRROAP> detMaquina = MaestroRoles;

                if (detMaquina != null && !detMaquina[0].GRUSCOGR.Equals(""))
                {
                    GRROAP maq = detMaquina.Find(x => x.GRUSCOGR == corrLiq);

                    descripcion3Label.Text = maq.GRUSCOGR.Trim(); 
                    localizacionLabel.Text = maq.GRUSDEGR.Trim();

                    codgruHiddenField.Value = maq.GRUSCOGR.ToString();
                    codaplHiddenField.Value = maq.ROAPCOAP;
        ...
        private bool ValidaDatos(out string mensajeValidacion)
        {
            ...
            if (string.IsNullOrWhiteSpace(codgrueditTextBox.Text))
            {
                mensajeValidacion = Mensajes.MENSAJE_INGRESE_CODIGO;
                return false;
            }

            if (string.IsNullOrWhiteSpace(desgrueditTextBox.Text))
            {
        ...
        protected void agregarButton_Click(object sender, EventArgs e)
        {
            try
            {
                tituloeditLiteral.Text = Mensajes.TITULO_AGREGAR_ROL;
                codgruHiddenField.Value = "";
                codgrueditTextBox.Text = "";
                desgrueditTextBox.Text = "";
            ...
            try
            {
                ...
                if (row != null)
                {
                    ...
                    if (detMaquina != null && !detMaquina[0].GRUSCOGR.Equals(""))
                    {
                        ...
                        codgrueditTextBox.Text = maq.GRUSCOGR.Trim();
                        desgrueditTextBox.Text = maq.GRUSDEGR.Trim();

                        estacteditCheckBox.Checked = maq.ROAPESTA.Equals(Constantes.ESTADO_ACTIVO);

                        tituloeditLiteral.Text = Mensajes.TITULO_MODIFICAR_ROL; 
        ...
        protected void aceptarBusqButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    End_Block(panelEdit); //udpInnerUpdatePanel);
                    CargaRoles(Convert.ToInt32(pageIndexHiddenField.Value));
                    MuestraDatos();
                }
            }
            catch (Exception ex)
        ...
        private bool GuardaVariable()
        {
            ...
            try
            {
                ...
                //codigo de operacion
                PAROPE argumentos = new PAROPE();
                argumentos.CODOPE = CodigoOperacion.GUARDA_ROLE;
                //asigna parametros entrada en orden
                //asigna parametros entrada en orden

                maq.GRUSCOGR = codgrueditTextBox.Text.Trim();
                maq.GRUSDEGR = desgrueditTextBox.Text.Trim();

                maq.ROAPESTA = estacteditCheckBox.Checked ? Constantes.ESTADO_ACTIVO : Constantes.ESTADO_ELIMINADO_NO_ACTIVO;
            ...
            try
            {
                ...
                if (row != null)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(corrLiq) && !corrLiq.Equals(""))
                    {
                        ...
                        maq.GRUSCOGR = corrLiq; // codgruHiddenField.Value; // codgrueditTextBox.Text.Trim();
                        maq.ROAPCOAP = row.Cells[4].Text; //codaplHiddenField.Value;

                        maq.ROAPESTA = row.Cells[2].Text;



                        //codigo de operacion
                        PAROPE argumentos = new PAROPE();
                        argumentos.CODOPE = CodigoOperacion.ELIMINA_ROLE;
                        //asigna parametros entrada en orden
