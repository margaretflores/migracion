...
{
    public partial class estandarescalidades : System.Web.UI.Page
    {
        ...
        private List<PCTEST> EstandaresCalidades
        {
            get
            {
                List<PCTEST> dt = HttpContext.Current.Session[Constantes.NOMBRE_SESION_TABLA_ESTANDARES_CALID] as List<PCTEST>;
                if (dt == null)
                {
                    // Create a DataTable and save it to session
                    dt = ObtieneCabeceraDefault();

                    EstandaresCalidades = dt;
                }
                return dt;
            }
            set
            {
                HttpContext.Current.Session[Constantes.NOMBRE_SESION_TABLA_ESTANDARES_CALID] = value;
            }
        }
        ...
        private void CargaEstandares(int pageindex)
        {
            ...
            try
            {
                ...
                parEnt.Add(Constantes.MOSTRAR_CALIDADES);
        ...
        private void SetDataSource(List<PCTEST> _listaMaquinas)
        {
            ...
            EstandaresCalidades = _listaMaquinas;
            cabeceraGridView.DataSource = EstandaresCalidades;
        ...
        private void MuestraDatos()
        {
            ...
            if (row != null)
            {
                ...
                //LENAR TABPAGES
                List<PCTEST> detMaquina = EstandaresCalidades;
                ...
                if (detMaquina != null && detMaquina[0].TESTIDTE != -1)
                {
                    ...
                    calidadLabel.Text = maq.TESTCALI + " " + maq.DECADESC;
        ...
        private bool ValidaDatos(out string mensajeValidacion)
        {
            ...
            {
                if (string.IsNullOrWhiteSpace(valor2editTextBox.Text) || !decimal.TryParse(valor2editTextBox.Text, out valor2))
                {
        ...
        protected void cabeceraGridView_PageIndexChanging(object sender, System.Web.UI.WebControls.GridViewPageEventArgs e)
        {
            try
            {
                ...
                SetDataSource(EstandaresCalidades);
            }
            catch 
            { 
            }
        ...
        protected void agregarButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                calidDropDownList.SelectedIndex = 0;
            ...
            try
            {
                ...
                if (row != null)
                {
                    ...
                    if (detMaquina != null && detMaquina[0].TESTIDTE != -1)
                    {
                        ...
                        if (calidDropDownList.Items.FindByValue(maq.TESTCALI.ToString()) != null)
                        {
                            calidDropDownList.SelectedValue = maq.TESTCALI;
                        }
        ...
        private bool GuardaEstandar()
        {
            ...
            try
            {
                ...
                maq.TESTCALI = calidDropDownList.SelectedValue;
                maq.TESTTPFI = "";
                maq.TESTTBLV = "";
                maq.TESTTCLV = "";
        ...
        public void CargaCalidades()
        {
            ...
            try
            {
                ...
                //codigo de operacion
                PAROPE argumentos = new PAROPE();
                argumentos.CODOPE = CodigoOperacion.OBTIENE_CALIDADES_FILTRO;
                //asigna parametros entrada en orden
                List<string> parEnt = new List<string>();
                argumentos.VALENT = parEnt.ToArray();
                resultado = clt.EjecutaOperacion(argumentos);
                if (resultado.ESTOPE)
                {
                    if (resultado.VALSAL[0].Equals("1")) //encontrado
                    {
                        var firstitem = calidDropDownList.Items[0];
                        calidDropDownList.Items.Clear();
                        calidDropDownList.Items.Add(firstitem);

                        List<PRCADE> datos = FuncionesUtil.Deserialize<List<PRCADE>>(resultado.VALSAL[1]);
                        foreach (PRCADE item in datos)
                        {
                            calidDropDownList.Items.Add(new ListItem(item.CALICODI.Trim() + " " + item.DECADESC, item.CALICODI));
                        }
                    }
                }
                else
