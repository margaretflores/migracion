...
{
    public partial class calidadgrupopendientes : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (!Page.IsPostBack)
                    {
                        CargaTablasTiposGrupoPendMaquina();
                        //
                        CargaDatosIniciales();
                    }
                }
        ...
        {
            if (tablastiposDropDownList.SelectedValue.Equals(Constantes.CODIGO_LISTA_SELECCIONE))
            {
                SetDataSource(null);
            }
            else
        ...
        public void CargaTablasTiposGrupoPendMaquina()
        {
            ...
            try
            {
                ...
                parEnt.Add(tablastiposHiddenField.Value);
                ...
                if (resultado.ESTOPE)
                {
                    ...
                    {
                        var firstitem = tablastiposDropDownList.Items[0];
                        tablastiposDropDownList.Items.Clear();
                        tablastiposDropDownList.Items.Add(firstitem);

                        List<PPTGPM> datos = FuncionesUtil.Deserialize<List<PPTGPM>>(resultado.VALSAL[1]);
                        foreach (PPTGPM item in datos)
                        {
                            tablastiposDropDownList.Items.Add(new ListItem(item.TGPMDESC, item.TGPMCODI));
                        }
                    }
        ...
        protected void tablastiposDropDownList_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (tablastiposDropDownList.SelectedValue.Equals(Constantes.CODIGO_LISTA_SELECCIONE))
                {
                    tbgrpeHiddenField.Value = "";
                    SetDataSource(null);
                }
                else
                {
                    tbgrpeHiddenField.Value = tablastiposDropDownList.SelectedValue;
                    CargaCalidadesTipoGrupoPendientes(1);
                    CargaTiposGrupoPendMaquina();
                }
            }
